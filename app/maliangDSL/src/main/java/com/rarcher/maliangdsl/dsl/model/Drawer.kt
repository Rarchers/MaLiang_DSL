...
//                    painterMap["default"]!!.measureText((pathMap[this]!! as TextComponentBean).text)
    ...
    infix fun ChineseBean.上边距(top: Float): ChineseBean {
        if (this.place == 位置.左下角 || this.place == 位置.右下角 || this.place == 位置.正中 )
            return this
        else
            this.topEdge = top
        return this
    ...
    infix fun ChineseBean.下边距(bottom: Float): ChineseBean {
        if (this.place == 位置.左上角 || this.place == 位置.右上角 || this.place == 位置.正中)
            return this
        else
    ...
    infix fun ChineseBean.左边距(left: Float): ChineseBean {
        if (this.place == 位置.右下角 || this.place == 位置.右上角 || this.place == 位置.正中 )
            return this
        else
            this.leftEdg = left
        return this
    ...
    infix fun ChineseBean.右边距(right: Float): ChineseBean {
        if (this.place == 位置.左上角 || this.place == 位置.左下角 || this.place == 位置.正中 )
            return this
        else
            this.rightEdg = right
        return this
    ...
    fun drawComponent(idPaint: String, idComponent: String) {
        if (!painterMap.containsKey(idPaint)) {
            ...
            when (pathMap[idComponent]!!.type) {
                ...
                ComponentType.CIRCLE ->{
                    val bean = (pathMap[idComponent]!! as CycleComponentBean)
                    bean.let {
                        workQueue.add(
                            cyclePacket(WorkType.FREE,it,idPaint)
                        )
                    }
                }
